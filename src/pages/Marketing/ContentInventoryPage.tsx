    ...
    <>
      ...
      <div className="body">
        ...
        {visible.length > 0 && (
          ...
          >
            {visible.map((p) => (
              <ProjectCard key={p.id} p={p} isAr={isAr} />
            ))}
          </div>
        )}
      </div>
...
function ProjectCard({ p, isAr }: { p: ContentInventoryProject; isAr: boolean }) {
  const natureSegs = [
    { key: 'real', n: p.by_nature.real, color: NATURE.real, label: isAr ? 'حقيقية' : 'Real' },
    { key: 'ai', n: p.by_nature.ai, color: NATURE.ai, label: isAr ? 'ذكاء اصطناعي' : 'AI / CGI' },
    { key: 'graphic', n: p.by_nature.graphic, color: NATURE.graphic, label: isAr ? 'تصاميم' : 'Graphics' },
    {
      key: 'other',
      n: p.by_nature.screenshot + p.by_nature.unknown,
      color: NATURE.other,
  ...
  const kinds: Array<[string, number]> = [
    [isAr ? 'صور' : 'Images', p.by_kind.image],
    [isAr ? 'فيديو' : 'Videos', p.by_kind.video],
    [isAr ? 'PDF' : 'PDFs', p.by_kind.pdf],
    [isAr ? 'مستندات' : 'Docs', p.by_kind.document],
    [isAr ? 'أخرى' : 'Other', p.by_kind.other],
  ];
  const roles: Array<[string, number]> = [
    [isAr ? 'المعرض' : 'Gallery', p.by_role.gallery],
    [isAr ? 'تسويقية' : 'Marketing', p.by_role.marketing],
    [isAr ? 'رئيسية' : 'Main', p.by_role.main],
    [isAr ? 'من المطوّر' : 'Developer', p.by_role.developer],
    [isAr ? 'أخرى' : 'Other', p.by_role.other],
  ];
      ...
      {empty ? (
        ...
        <>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            ...
            {kinds.filter(([, n]) => n > 0).map(([label, n]) => (
              <Chip key={label}>{label} · {num(n, isAr)}</Chip>
            ))}
          </div>

          {/* nature split — the AI-vs-real question, as a bar + legend */}
          {natureTotal > 0 && (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
              <div
                style={{ display: 'flex', height: 8, borderRadius: 999, overflow: 'hidden', background: 'color-mix(in srgb, var(--ink, #4A4E54) 8%, transparent)' }}
                aria-hidden
              >
                {natureSegs.map((s) => (
                  <span key={s.key} style={{ width: `${(s.n / natureTotal) * 100}%`, background: s.color }} />
                ))}
              </div>
              <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                {natureSegs.map((s) => (
                  <span key={s.key} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: 11.5, color: 'var(--mute)' }}>
          ...
          {/* role split */}
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {roles.filter(([, n]) => n > 0).map(([label, n]) => (
              <Chip key={label} tone="soft">{label} · {num(n, isAr)}</Chip>
            ))}
          </div>
...
function Chip({ children, strong, tone }: { children: React.ReactNode; strong?: boolean; tone?: 'soft' }) {
  return (
    <span
      style={{
        fontSize: 11.5,
