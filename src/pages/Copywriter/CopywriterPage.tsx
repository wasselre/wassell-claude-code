...
import { PenLine, Plus, MessageSquare } from 'lucide-react';
import type { AppRecord } from '@/types';
import CopywriterThread from './components/CopywriterThread';

/**
 * Two-pane "Copywriter" layout. Left = list of past conversations (records
 * in the `copywriter_chats` model). Right = active chat thread or an
 * empty-state welcome card. Routes `/model/copywriter_chats` and
 * `/model/copywriter_chats/:recordId` both land here via the dispatcher in
 * App.tsx.
 *
 * Same shape as the AI sales agent (src/pages/AiAgent) — messages for each
 * conversation live inline in `record.data.messages`; the backend
 * (api/copywriter.ts) is stateless and gets the full text history each turn.
 */
export default function CopywriterPage() {
  ...
  const model = useMemo(() => models.find((m) => m.name === 'copywriter_chats'), [models]);
  ...
  function startNewChat() {
    ...
    saveRecord(record);
    navigate(`/model/copywriter_chats/${newId}`);
  }
  ...
  if (!model) {
    return (
      <div className="p-8 text-center text-charcoal/70">
        {isAr
          ? 'نموذج كاتب المحتوى غير مهيأ بعد. أعد تحميل الصفحة.'
          : 'Copywriter model not initialized. Please reload.'}
      </div>
    );
  }
  ...
  return (
    <div className="h-[calc(100vh-8rem)] flex overflow-hidden rounded-2xl border border-sand/20 bg-white">
          ...
          {chats.length === 0 ? (
            ...
            chats.map((chat) => {
              ...
              return (
                <button
                  key={chat.id}
                  onClick={() => navigate(`/model/copywriter_chats/${chat.id}`)}
      ...
      <div className={`flex-1 min-w-0 flex-col ${!recordId ? 'hidden md:flex' : 'flex'}`}>
        {recordId ? (
          <CopywriterThread key={recordId} recordId={recordId} modelId={model.id} onNewChat={startNewChat} />
        ) : (
  ...
  return (
    <div className="flex-1 flex flex-col items-center justify-center p-8 text-center">
      <div className="w-16 h-16 rounded-full bg-copper/10 flex items-center justify-center mb-4">
        <PenLine size={32} className="text-copper" />
      </div>
      ...
      <p className="text-sm text-charcoal/70 max-w-md mb-6">
        {isAr
          ? 'كاتب محتوى متخصص يكتب نصوص ريلز عقارية بأسلوب أقوى المسوّقين السعوديين، مستنداً إلى مكتبة المنافسين ومشاريعنا. اطلب نص ريل، أو حسّن نصاً، أو اطلب خطّافات.'
          : 'A specialist copywriter that writes real-estate reel scripts in the style of the strongest Saudi marketers, grounded in the competitor library + our projects. Ask for a reel script, improve a draft, or generate hooks.'}
      </p>
      <button
        onClick={onStart}
