...
export default function UnitsTabPane({ projectId }: UnitsTabPaneProps) {
  ...
  // Pagination — local to the tab. Reset to page 1 whenever the unit set size
  // changes (project switch, view filter, add/delete).
  const [pageSize, setPageSize] = useState(25);
  const [currentPage, setCurrentPage] = useState(1);
  useEffect(() => {
    setCurrentPage(1);
  }, [scopedUnits.length, activeViewId]);
  const pagedUnits = useMemo(() => {
    const start = (currentPage - 1) * pageSize;
    return scopedUnits.slice(start, start + pageSize);
  }, [scopedUnits, currentPage, pageSize]);
      ...
      {scopedUnits.length === 0 ? (
        ...
        <>
          ...
          <TableView
            model={unitsModel}
            records={pagedUnits}
            onRowClick={(rec) => navigate(`/model/${unitsModel.name}/${rec.id}`)}
