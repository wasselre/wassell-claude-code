...
import {
  ...
  visitsId,
  fuWhatsAppSectionId,
  fuPostVisitSectionId,
  fuCallToBookSectionId,
} from '@/data/seedModels';
...
/**
 * Migration 10 → 11: Follow-Ups overhaul — single consolidated `result` field,
 * conditional reason fields, rescheduling sub-flow, related-records inline lists,
 * Call-to-Book Appointment section, and the rename of "Book a visit" →
 * "Schedule an appointment".
 *
 * Schema side: replaces the followups model schema + card_config with the
 * fresh seed shape. The four per-section result fields (call_result was kept
 * but moved to the base section; appt_call_result / pv_call_result /
 * whatsapp_result are deleted) and the legacy `next_followup_after_days`
 * field are removed.
 *
 * Data side: consolidates legacy result values into `call_result` (first
 * non-null wins, priority appt → pv → whatsapp). Maps the whatsapp-only
 * `no_reply` to `no_answer`; whatsapp-only slugs without a CALL_RESULT_OPTIONS
 * equivalent (`replied`, `needs_info`, `will_reply_later`) are dropped to
 * blank — the agent re-picks on next edit. Backfills `status = 'active'` for
 * every existing follow-up so D2's cancel-cascade has a clean baseline.
 *
 * Idempotent: any record that already has `call_result` set is left alone
 * (we don't overwrite a deliberate value with a legacy one).
 */
const migration_10_to_11: Migration = (state) => {
  const followupsSeed = SEED_MODELS.find((m) => m.name === 'followups');
  if (!followupsSeed) return state;

  const idx = state.models.findIndex((m) => m.name === 'followups' && m.is_system);
  ...
  // Map a legacy slug to the new shared `call_result` slug.
  // Most call-result slugs carry over unchanged. `no_reply` was a
  // whatsapp-only slug; map it to `no_answer`. The other whatsapp-only
  // values have no equivalent in CALL_RESULT_OPTIONS and become blank.
  const mapLegacy = (v: unknown): string | null => {
    if (typeof v !== 'string' || v === '') return null;
    if (v === 'no_reply') return 'no_answer';
    if (v === 'replied' || v === 'needs_info' || v === 'will_reply_later') return null;
    return v;
  };
  ...
  if (existingRecords.length) {
    records[existing.id] = existingRecords.map((rec) => {
      ...
      // Consolidate to call_result (first non-null wins, priority order).
      if (typeof data.call_result !== 'string' || data.call_result === '') {
        const candidate =
          mapLegacy(data.appt_call_result) ??
          mapLegacy(data.pv_call_result) ??
          mapLegacy(data.whatsapp_result);
        if (candidate) {
          data.call_result = candidate;
          changed = true;
        }
      }
      // Drop the now-deleted slugs unconditionally (idempotent: no-op if absent).
      if ('appt_call_result' in data) { delete data.appt_call_result; changed = true; }
      if ('pv_call_result' in data)    { delete data.pv_call_result;    changed = true; }
      if ('whatsapp_result' in data)   { delete data.whatsapp_result;   changed = true; }
      if ('next_followup_after_days' in data) { delete data.next_followup_after_days; changed = true; }

      // Default new `status` field to 'active' so D2's cancel-cascade has
      // a sensible baseline. Pre-existing status (unlikely — field is new)
      // is preserved.
      ...
      if (changed) migrated++;
      return changed ? { ...rec, data, updated_at: new Date().toISOString() } : rec;
    });
  }

  if (migrated > 0) {
    console.warn(
      `[schemaMigrations v11] Followups overhaul: re-applied seed schema, migrated ${migrated} record(s) ` +
        '(consolidated call_result, backfilled status, dropped legacy result slugs).',
    );
  } else {
    console.warn('[schemaMigrations v11] Followups overhaul: re-applied seed schema (no record data changes).');
  }

  // Seed the D1 (WhatsApp next follow-up) and D3 (follow-up reschedule
  // write-back) workflows. They're keyed by label so the migration is
  // idempotent — re-runs skip any workflow that's already present.
  // D2 (cancel-cascade on appointment date change) is intentionally NOT
  // seeded here — it needs a multi-record update primitive that doesn't
  // exist yet (current `update_record` matches one row at a time). Tracked
  // as future work in docs/prd/followups.md.
  // Workflow seeding intentionally lives in `healFollowupWorkflows` below
  // (always-run, idempotent) instead of here. This migration only fires when
  // the schema version is below 11, but React StrictMode + concurrent
  // `initialize()` calls can race so the workflow-seeding branch is missed
  // by the second call — moving the seed to a heal pass that runs every
  // boot makes the seeded workflows survive the race.
  return { ...state, models, records };
};

/**
 * Always-run heal that ensures the two seeded follow-up workflows (D1 and D3)
 * are present. Idempotent: matches by `label_en`, so user edits to the
 * workflows via the Builder are preserved. D2 (appointment-date cancel cascade)
 * is not seeded — needs a multi-record update primitive that doesn't exist yet.
 */
export function healFollowupWorkflows(workflows: Workflow[]): { workflows: Workflow[]; changed: boolean } {
  const next = seedFollowupWorkflows(workflows);
  return { workflows: next, changed: next.length !== workflows.length };
}

/**
 * Seed the D1 and D3 workflows on the followups model. Idempotent by label
 * — any workflow whose `label_en` is in our seed set is left untouched, so
 * the user can edit it via the Builder without the next migration run
 * clobbering their changes. Only adds missing workflows.
 */
function seedFollowupWorkflows(existing: Workflow[]): Workflow[] {
  const now = new Date().toISOString();
  const out = existing.slice();
  const existingLabels = new Set(existing.map((w) => w.label_en));

  // D1 — On WhatsApp follow-up save where next_followup_datetime + type are
  // set, create the next follow-up. Branches by next_followup_type and
  // (for 'recall') by whether the client has any prior visit.
  const D1_LABEL = 'WhatsApp → Schedule next follow-up';
  if (!existingLabels.has(D1_LABEL)) {
    const d1: Workflow = {
      id: uuid(),
      label_ar: 'واتساب ← جدولة المتابعة التالية',
      label_en: D1_LABEL,
      trigger_model_id: followupsId,
      trigger_event: 'update',
      branches: [
        {
          id: uuid(),
          label_en: 'Next type = WhatsApp',
          condition_mode: 'all',
          conditions: [
            { id: uuid(), field_id: 'next_followup_datetime', operator: 'is_not_empty', value: null, only_on_change: true },
            { id: uuid(), field_id: 'next_followup_type',     operator: 'equals',       value: 'whatsapp' },
          ],
          actions: [
            {
              id: uuid(),
              ...
              field_mappings: [
                { id: uuid(), target_field_id: 'scheduled_datetime', source_type: 'trigger_field', trigger_field_id: 'next_followup_datetime' },
                { id: uuid(), target_field_id: 'followup_type',      source_type: 'static',         static_value: [fuWhatsAppSectionId] },
                { id: uuid(), target_field_id: 'client_id',          source_type: 'trigger_field', trigger_field_id: 'client_id' },
                { id: uuid(), target_field_id: 'status',             source_type: 'static',         static_value: 'active' },
              ],
            },
          ],
        },
        {
          id: uuid(),
          label_en: 'Next type = Recall, has visits → Post-Visit',
          condition_mode: 'all',
          conditions: [
            { id: uuid(), field_id: 'next_followup_datetime', operator: 'is_not_empty', value: null, only_on_change: true },
            { id: uuid(), field_id: 'next_followup_type',     operator: 'equals',       value: 'recall' },
            {
              id: uuid(),
              field_id: 'client_id',
              operator: 'exists_related_record',
              value: null,
              related_model_id: visitsId,
              related_match_field_id: 'client_id',
            },
          ],
          actions: [
            {
              ...
              field_mappings: [
                { id: uuid(), target_field_id: 'scheduled_datetime', source_type: 'trigger_field', trigger_field_id: 'next_followup_datetime' },
                { id: uuid(), target_field_id: 'followup_type',      source_type: 'static',         static_value: [fuPostVisitSectionId] },
                { id: uuid(), target_field_id: 'client_id',          source_type: 'trigger_field', trigger_field_id: 'client_id' },
                { id: uuid(), target_field_id: 'status',             source_type: 'static',         static_value: 'active' },
              ],
            },
        ...
        {
          id: uuid(),
          label_en: 'Next type = Recall, no visits → Call to Book',
          is_else: true,
          conditions: [],
          actions: [
            {
              ...
              field_mappings: [
                { id: uuid(), target_field_id: 'scheduled_datetime', source_type: 'trigger_field', trigger_field_id: 'next_followup_datetime' },
                { id: uuid(), target_field_id: 'followup_type',      source_type: 'static',         static_value: [fuCallToBookSectionId] },
                { id: uuid(), target_field_id: 'client_id',          source_type: 'trigger_field', trigger_field_id: 'client_id' },
                { id: uuid(), target_field_id: 'status',             source_type: 'static',         static_value: 'active' },
              ],
            },
  ...
  // D3 — When the agent sets a rescheduled date on a follow-up, push it back
  // to the linked appointment record. That appointment update would (once D2
  // is implemented) cascade cancellation to all related follow-ups.
  const D3_LABEL = 'Follow-up rescheduled → Update appointment';
  if (!existingLabels.has(D3_LABEL)) {
    const d3: Workflow = {
      id: uuid(),
      label_ar: 'إعادة جدولة المتابعة ← تحديث الموعد',
      label_en: D3_LABEL,
      trigger_model_id: followupsId,
      trigger_event: 'update',
      branches: [
        {
          id: uuid(),
          label_en: 'Apply reschedule to linked appointment',
          condition_mode: 'all',
          conditions: [
            { id: uuid(), field_id: 'call_result',                  operator: 'equals',       value: 'rescheduled' },
            { id: uuid(), field_id: 'appointment_id',               operator: 'is_not_empty', value: null },
            { id: uuid(), field_id: 'rescheduled_appointment_date', operator: 'is_not_empty', value: null, only_on_change: true },
          ],
          actions: [
            {
              id: uuid(),
              ...
              filter_value: null,
              filter_trigger_field_id: 'appointment_id',
              field_mappings: [
                { id: uuid(), target_field_id: 'appointment_date', source_type: 'trigger_field', trigger_field_id: 'rescheduled_appointment_date' },
                { id: uuid(), target_field_id: 'project_id',       source_type: 'trigger_field', trigger_field_id: 'rescheduled_appointment_project' },
              ],
            },
          ],
        },
