...
/**
 * Map a `ChatModelId` + whether the turn carries input images to its fal.ai
 * endpoint slug (all env-overridable).
 *
 * IMPORTANT: the `/edit` variants REQUIRE a non-empty `image_urls` — fal returns
 * `422 {"loc":["body","image_urls"],"msg":"Field required"}` without it. So a
 * turn with NO attachments (the common "new chat, just type a prompt" case)
 * must use the text-to-image (non-`/edit`) slug. nano-banana-pro's t2i slug is
 * the same one the icon flow already uses (`readIconEnv`), so it's known-good.
 */
function resolveChatModelSlug(id: ChatModelId, hasImages: boolean): string {
  switch (id) {
    case 'gpt-image-2':
      return hasImages
        ? (process.env.FAL_CHAT_GPT_IMAGE_2_MODEL_ID ?? 'openai/gpt-image-2/edit')
        : (process.env.FAL_CHAT_GPT_IMAGE_2_T2I_MODEL_ID ?? 'openai/gpt-image-2');
    case 'nano-banana':
    default:
      return hasImages
        ? (process.env.FAL_CHAT_MODEL_ID ?? 'fal-ai/nano-banana-pro/edit')
        : (process.env.FAL_CHAT_T2I_MODEL_ID ?? 'fal-ai/nano-banana-pro');
  }
}
 ...
 */
function readChatEnv(modelId: ChatModelId = 'nano-banana', hasImages = true): FalEnv {
  const apiKey = process.env.FAL_KEY;
  if (!apiKey) {
    throw new Error('FAL_KEY is not set');
  }
  return {
    apiKey,
    baseUrl: process.env.FAL_BASE_URL ?? 'https://queue.fal.run',
    modelId: resolveChatModelSlug(modelId, hasImages),
  };
}
...
export async function imageGenChat(opts: ChatGenOpts): Promise<ImageGenStartResult> {
  const hasImages = opts.imageUrls.length > 0;
  const env = readChatEnv(opts.modelId ?? 'nano-banana', hasImages);
  if (env.apiKey === STUB_KEY) {
...
interface TextRemovalOpts {
  /** URL of the source photo. Downloaded worker-side and inlined as a data URI. */
  imageUrl: string;
  signal?: AbortSignal;
}
