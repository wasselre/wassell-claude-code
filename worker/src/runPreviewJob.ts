...
/** Hard ceiling on a single soffice run. Typical conversions are 2–10 s;
 *  this is the runaway backstop (the SQL watchdog is the crash backstop). */
...
export async function runPreviewJob({ supabase, env, job }: RunPreviewJobArgs): Promise<string> {
  ...
  try {
    ...
    try {
      ...
      throw new Error(`soffice conversion failed: ${msg}`);
    }
