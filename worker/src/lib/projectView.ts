// WORKER COPY of src/lib/projects/projectView.ts — the PURE resolver for the
// Projects + Units experience. The worker is a standalone npm package and cannot
// import `@/` paths, so this is a straight port with the two aliased imports
// repointed at the worker's local copies (localizedName.js, projectTypes.js).
// Keep semantically identical to the canonical resolver — the unit one-pager PDF
// must say EXACTLY what the rep's on-screen inventory says.

import { resolveLocalizedName, resolveArabicName, pickLocalized, type LocalizedName } from './localizedName.js';
import type { AppModel, AppRecord, ModelField, FieldOption } from './projectTypes.js';
...
/**
 * Bilingual W6: optional language + translation resolver for the view. Injected
 * (never imported) so this module stays server-safe. Absent ⇒ Arabic + source
 * text.
 */
...
/** Geography display in the requested language; Arabic fallback. */
function geoDisplay(
  store: ProjectStoreSlices,
  modelName: 'cities' | 'districts',
  id: string | null,
...
export function resolveProjectView(
  ...
  const isAr = opts.isAr !== false; // default Arabic
