/**
 * Video-script generator — WORKER COPY of api/_lib/marketing/videoScript.ts.
 *
 * The worker is a standalone npm package (rootDir:src; the Dockerfile copies
 * only src/) and CANNOT import from api/_lib — same posture as
 * worker/src/imageGen.ts. Keep this file in EXACT sync with
 * api/_lib/marketing/videoScript.ts (recipes, rules, prompt, model, mapping).
 * The skill (.claude/skills/writing-video-script/SKILL.md) is the source spec.
 */
