/**
 * ⚠️  KEEP IN SYNC with api/_lib/migrateAgent.ts — this is a VERBATIM COPY.
 * The Fly worker is a standalone npm package (rootDir:src; the Dockerfile
 * compiles only worker/src) and CANNOT import from api/_lib, so the extraction
 * agent is copied here (same convention as worker/src/imageGen.ts). When you
 * change extraction logic, model IDs, prompts, tools, or the error class in the
 * original, change BOTH files. The worker drains the file-heavy actions (extract
 * / discover / fuse_batch / plan / discuss) via worker/src/runMigrationJob.ts;
 * the API still imports the original for the fast synchronous actions
 * (suggest_mappings / standardize).
 *
