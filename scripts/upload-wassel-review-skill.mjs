// One-time / re-runnable upload of the wassel-deck-review skill folder
// to the Anthropic Skills API. Prints the skill_id to wire into Vercel env.
//
// Run: $env:ANTHROPIC_API_KEY = "sk-ant-..."; node scripts/upload-wassel-review-skill.mjs
//
...
//
// Pairs with `scripts/upload-wassel-skill.mjs` (the builder skill). The
// production endpoint loads BOTH skill_ids — wassel-general-ppt for
// composition + wassel-deck-review as the auto-patch QA gate before
// returning the file.
...
const SKILL_DIR = "C:\\Users\\rayan\\.claude\\skills\\wassel-deck-review";
const SKILL_ROOT_PREFIX = "wassel-deck-review"; // common root in upload paths
const DISPLAY_TITLE = "Wassel Deck Review";
...
async function main() {
  ...
  if (versionOf) {
    ...
    console.log(`  ANTHROPIC_WASSEL_REVIEW_SKILL_ID=${skill.id}`);
  }
}
