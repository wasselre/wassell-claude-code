...
4. **Ship DARK, activate deliberately.** Policies land `proposed`; nothing processes until a human `confirmed`s them (or a migration does, for the cutover set). Resources gate capture; `translation_settings.is_enabled` gates BOTH the capture trigger AND `translation_job_claim_next` + `translation_reconcile` (the W2 kill-switch-bypass lesson: OFF must mean off everywhere). Twin pairs (`chat_templates`/`posts_content`) are built but their pair policies stay `proposed` — twin go-live = confirm the pair policy + run `translation_twin_backfill(model_id)`.

5. **Twins (`pair:*`) materialize into the PHYSICAL column, not a variant.** `record_twin_fill` (CAS: source-sha unchanged AND target still empty/prior-machine-text) writes the other `_en`/`_ar` column under the `system_write='twin_fill'` GUC. v1 fills EMPTY sides only. Don't route twins through the variant/display path — the app reads the twin columns directly.
