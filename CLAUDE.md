...
3. **SOLE-WRITER rule (echo-dedup):** during a job the BROWSER must never write the migration record via the store — a browser write registers a null-`updated_at` entry in the realtime echo-dedup (`src/lib/realtime/dedup.ts`) that would SUPPRESS the worker's next update. So the busy/status flip + message append happen server-side in `/api/migrate` (service role), and all job-time writes are the worker's (via `record_save` + `p_expected_version` + retry-on-40001). The browser only reads via Realtime. (The `import` step is the lone exception — it writes the record from the tab, but no worker touches the record then.)
