...
## Never raise SQLSTATE 40001 / 40P01 from any function (added 2026-09-07 — ROOT CAUSE of every conflict storm)

**PostgREST runs every request inside hasql-transaction, which re-runs a transaction that aborts with SQLSTATE `40001` (serialization_failure) or `40P01` (deadlock_detected) FOREVER — no backoff, no attempt cap, and it keeps going after the client disconnects.** Until 2026-09-07 `record_save` raised `version_mismatch` and `conflict_storm_blocked` with ERRCODE 40001 (and the five `record_translation_*` CAS RPCs raised `source_changed`/`revision_mismatch` the same way). So ONE stale save from ONE browser tab became an unbounded server-side loop: ~1,200 rolled-back `record_save` executions per second, per request, for as long as the version stayed mismatched — 52 hours on 2026-09-05→07, 82 hours in July. The client never saw a single error (its one HTTP request was still open), which is why no client-side breaker, cap, or `record_conflict_report` telemetry ever fired; closing the tab / revoking the session did nothing (the loop lives in PostgREST); and the only thing that ever ended a storm was a `noop`/`noop_stale` block turning the failure into a success. Proof: one `curl` with `p_expected_version=-1` produced several conflict log lines within 17 ms, the sweep saw that row's xmax 57,578 transactions ahead of every other row and auto-blocked it, and only then did the curl get its 200.

**Hard rules — never violate:**
1. **No function in `public` may raise SQLSTATE `40001` or `40P01`** (by code or by condition name `serialization_failure` / `deadlock_detected`). Use **`WS409`** for optimistic-concurrency conflicts (`version_mismatch`, `source_changed`, `revision_mismatch`) and **`WS429`** for rate-limit blocks (`conflict_storm_blocked`). PostgREST maps an unknown SQLSTATE to HTTP 400 and the request completes immediately. Enforced by the assertion in `supabase/migrations/2026-09-07_never_raise_sqlstate_40001.sql` and by `supabase/tests/ci/assert_no_retryable_sqlstate.sql` in the `db-migrations` CI job.
2. **Consumers classify by MESSAGE first, code second** (`version_mismatch` / `conflict_storm_blocked` / `source_changed`), and accept both the new code and the legacy `40001` (`src/stores/appStore.ts` `supabaseRecordUpsert`, `api/_lib/recordSaveRetry.ts` + its `worker/src/lib` copy, `api/_lib/workflowSweeper.ts`).
3. **Do not "fix" a storm by adding client-side brakes first.** The full incident history and the layered defenses (per-record `noop_stale` blocks, session blocks, the rollback-rate sweep) are in `docs/conflict-storm-hardening.md` §7; they remain as containment, but the mechanism they were fighting is gone.
...
- Writes go through the **`record_save(model_id, id, data, created_by, expected_version)` / `record_delete(model_id, id)` SQL RPCs** which dispatch on `is_hardcoded`. The frontend store wraps these in `supabaseRecordUpsert` / `supabaseRecordDelete`. `p_created_by` is COALESCE-preserved on update — first save stamps, later edits don't overwrite. The 5th arg `p_expected_version` (Phase F.2, added 2026-05-07) is the version the client loaded with; on mismatch the RPC raises `version_mismatch` as SQLSTATE **WS409** (NEVER 40001 — see "Never raise SQLSTATE 40001" below) and the frontend surfaces a "reload to see latest" toast instead of overwriting concurrent edits. Frozen models ignore p_expected_version (no per-frozen-table version column yet — Phase F.2.1 future work). Same posture as the records-table flow.
...
- **Payment-plan menu rides the same trigger (added 2026-09-07, `supabase/migrations/2026-09-07_project_payment_plans_rollup.sql`):** `tg_records_fill_project_rollups` ALSO merges `recalc_project_payment_plans_data(NEW.id)` — the distinct payment structures across the project's units' `payment_plans` cards → `payment_plan_schedule` (deterministic `_row_id` = md5(project id + structure key), so a no-change recompute rewrites nothing), `payment_plan_summary`, and the headline `down_payment_percent` / `during_construction_percent` / `on_handover_percent` from the ENTRY plan (lowest down %, tie-break highest on-handover %). **Manual fallback:** the function returns `'{}'` when NO unit has a card, so hand-typed project values survive only until the first unit gains one; `post_handover_months` is never derived. Both plan tables carry an optional free-text `schedule` column (milestone breakdown) that the rollup copies through. Don't add a client-side recompute of the menu (rule 5 below applies); the browser tab (`PaymentPlansTabPane`) aggregates the SAME cards live only to add per-structure price ranges, which the stored menu deliberately omits.
