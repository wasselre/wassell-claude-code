...
| `task_update` NEW | `{ task_id, due_at }` → `{ ok: true }` | s35 «تأجيل / تقديم» — moves an open task's due date. RLS gates to roles holding `assign` (manager / ops / admin). |
| `overview` CHANGED | `{ period?: 'week'\|'month'\|'quarter' }` → same shape + `period`, `unscheduled: [{id, ref, title, target_publish_at}]`, `waiting_oldest_at`, `late_mix: [{label_ar, label_en, n}]`; `week[]` rows gain `ref`/`title`. `week_start`/`week_end` carry the selected period's bounds. | s01's segmented control هذا الأسبوع/الشهر/الربع. |
| `work_list` CHANGED | `{ scope }` → same shape + `upcoming: [{content_id, ref, title, step_key, step_label_ar, step_label_en, steps_away}]` (mine scope only), and each task gains `is_approval` / `approval_kind` from the PINNED version's step. | `upcoming` = s02's «القادم إليك» band — future steps for the caller's role on in-flight items, derived from pinned-version paths; explicitly NOT tasks. Approval fields drive s35's creative/process split. |
| `ceo_overview` NEW | `{ period?: 'month'\|'quarter'\|'year' }` → `{ period, period_start, period_end, produced, produced_prev, spend, committed, leads, qualified, appointments, reservations, reservation_value, campaigns: [{id, ref, name, status, objective, starts_on, spend, qualified, reservations, cost_per_reservation}], production_by_month: [{month: 'YYYY-MM', count}] (last 6), pending_signature: [{id, ref, name, budget_total, success_metric, success_threshold, goal}], signature_threshold }` | s34. Outcomes aggregated from `mos_campaign_outcomes` over the listed campaigns; production from `published_at`; NO task data — the CEO is not a production manager. |
