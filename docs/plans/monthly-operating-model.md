...
Role holders in production: one marketing manager (you), one writer, one designer, and nobody at all holding the operations role that owned every publish check until three days ago. A second montage account exists under your own name and is not a second designer; it is yours, and it loses the montage role. Five roles were designed; one carries every approval and two carry nothing.
...
|---|---|---|---|---|
| Writer (a post with its caption is one slot; a row is one task counted as three) | 10 | 114 | 4.4 | 44% |
| Designer | 4, a floor they exceed | 114 | 4.4 | 110% of the floor |
| Marketing manager, writing review and final approval per row for organic and per creative for paid, a row counted as three | no limit, by decision | 228 slots (36 row approvals + 120 paid) | 8.8 slots | fits |

The writer has room. The designer is at the floor with about ten designs over the month, which you have said they absorb. Your approvals arrive as rows for organic and as creatives for paid; counted as three per row that is about nine slots a day, roughly six approval actions, and you have chosen not to cap that. So the month fits the current team.

Two things follow. The engine I built last week counts capacity with two knobs, slots per day and days per task, and I had seeded days per task at two for design; that silently halved the designer's throughput and produced a false "does not fit". The month screen must count throughput per person per day, one number each, and show the daily average next to the floor so you judge it yourself. And the one dependency no number removes is that every organic row and every paid creative passes through you twice; that is a fact about the design of the workflow, not a capacity problem.
...
Then the weekly refresh. It runs every minute, ranks creatives on real Meta numbers, though on Meta's lead count and a fixed window rather than our leads and each ad's own first seven days,, opens a decision task for you every week, and routes it to a screen that does not exist. The list and decide functions exist server-side with zero callers. With automatic application switched off, no cycle can ever complete. They accumulate in `deciding` forever.
...
- **The creative ranking's skeleton**: pure, unit-tested, the cost-per-lead arithmetic and the data gates. Its shape survives; its inputs and two verdicts change: our WhatsApp leads instead of Meta's, each ad's own first seven days instead of a fixed window, keep both inside the guards, cost per click when leads are too few, and keep-and-flag for fatigue on the only scaled ad (section 3.6).
...
Optionally, write a note on any row cell, project column, or the month itself before pressing confirm; nothing is required (section 3.7). That is the entire planning involvement. Five actions, one screen, once a month.
...
- A creative was rejected three times. Options: replace it with a new post, or move the row to the next slot. The row never goes out as two.
- An ad failed at Meta, with Meta's own message. Options: retry, or replace the creative.
- A publish failed, the account is not connected, or the platform rejected the post, with the platform's own message. Options: connect the account or fix what was named, then retry, or record it by hand.
...
|---|---|---|---|
| **Before** (the 17th onward) | choose three projects, add any notes to the grid, read one summary, confirm | nothing yet | compiles the month, reserves capacity, opens the first tasks on their start dates, builds the Meta campaigns |
| **During** | clear the exceptions list when it is not empty; review and approve rows (writing review and final approval per row, send-back per post) and paid creatives | write, design, review in My work exactly as now | advances tasks, builds and activates ads, publishes posts, runs the weekly refresh, raises exceptions |
| **After** (the 1st) | read the month page, which has become a report | nothing | closes the month, carries any unfinished work into the next month's first batch where it counts against that batch's capacity, sends the next reminder |
...
| Success measures editor, measure types, the main-measure flag | Two parallel measure models that only agree by accident. The month has two computed measures: cost per lead per project from our WhatsApp leads for the weekly ad rule, and cost per qualified lead per project for choosing next month's projects (section 3.6). |
...
| The publish tab per publication | Overriding the account for one destination. The caption is not overridden here: it comes from the approved writing and is checked against the approval hash at publish (section 3.4) |
...
| These | Into |
|---|---|
| Load & SLA + Capacity & calendar | One capacity screen that edits the three throughput numbers the runtime actually reads: writer, designer, manager, one per person per day. Publishing has no bucket; it is automatic |
| Publishing board + Release page + Publish tab | One release surface: the board lists, the release page acts, the tab is gone |
| Six approval surfaces | One row approval component used everywhere: writing review and final approval per row, send-back per post. Readiness is not an approval gate; it is refused at the designer's submit (section 3.4) |
| My work + My profile + Team | One work queue by person, not role, with a manager scope. One definition of "mine". |
| Overview + Analytics | One month page. It reads the real daily Meta table for spend and impressions and our WhatsApp leads for cost per lead, not the hand-keyed one |
| Ad modal + Tree modal | One ad editor, advanced only |

### 5.4 Retain unchanged

The Meta integration core, the creative ranking's pure core (re-fed with our leads and each ad's own seven days, section 3.6), the workflow engine (with its task subject widened from one content record to one row of three, section 3.5), the content record, the release sweep, bundle.social publishing, the competitor intelligence system, the attribution chain, the asset library, and the shoot requests.
...
| Ad activation | Part of the batch, not a human act. Created paused, activated on the batch date by the same lane that judges the running ads on their own first seven days, keeps the ad or ads the weekly rule keeps, flags a fatigued one, and pauses the rest (section 3.6). |
...
- The weekly refresh lane. Its ranking is required but must be re-pointed (section 3.6): our WhatsApp leads, each ad's own first seven days from activation, keep both inside the guards, cost per click when leads are too few, fatigue on the only scaled ad kept and flagged.
...
- The fifth-creative policy choice. Five new go live on every batch date; if the "banked spare" mechanism stays it is an internal production buffer that never reduces that five.
...
**Phase 0, stop the bleeding (days).** Add the ad activation step to the refresh lane so paid work stops ending in Ads Manager. Re-point the ranking to the settled rule (our WhatsApp leads over the same days as spend, each ad on its own first seven days from activation, the scale thresholds, keep both inside the guards, cost per click when leads are too few, fatigue on the only scaled ad kept and flagged, nothing deleted), then turn on automatic application. Nothing is escalated; there is no uncertain-ranking exception. Fix round zero. Delete the two dead builders and the orphaned template feature. Correct the stale approval copy that promises something the database refuses.

**Phase 1, the month screen (weeks).** One new page and one new settings screen, on top of the existing engine and commit, plus the material rule of section 3.4: releases read the two design slots by destination and the writing caption at publish, check the files against the approval hash, record the file ids and caption on the publication after posting, refuse a designer submit with an empty required slot, and publish the first-read post of a row last. The settings screen holds the standing month: the four posting weekdays, posts per row, paid creatives per project per week, campaign length, paid budget per project (2,000 riyals, 6,000 in all), the Meta template, lead times, and the four weekly-rule numbers: the scale thresholds (150 riyals, 2,000 impressions) and the guards (5 leads, 20 percent). The month page compiles it, shows the summary and the capacity verdict, and confirms. The commit creates the organic program, three paid campaigns, their executions with the Meta plan filled from the template, and their ad sets. Existing wizard, campaign pages and everything else stay exactly where they are. Run one real month this way.

**Phase 2, the exception list and batch approvals (weeks).** Route every exception source that already exists (incomplete row at its batch date, rejected three times, ad failed, publish failed, account not connected, platform rejected, sold-out project, capacity breach) into the one list on the month page. Add the November reminder, the notes on the month grid (row cell, project column, month) with the resolved brief on the row task, the sold-out exception with its replacement picker that takes over the remaining rows and slates, and the carry-over of unfinished work into the next month's first batch, counted against it. Build the row task: one task to work and one to approve with three post records inside, counted as three in capacity; writing review and final approval per row, send-back per post; order set by the writer, editable at approval, first-read post published last (section 3.5). Collapse the four "my work" definitions into one.
...
| Story | same day, same slot, the picture alone, no caption |
| Material | files from the two design slots by destination, caption from the writing task, resolved at publish, checked against the approval hash, recorded after; no file picker |
| Publishing | automatic; no publication task in the normal case; a task only when a person is needed: account not connected, platform rejected, ad failed, publish failed |
...
| Instructions | notes per row cell, per project column, and on the month, on the month grid, written at confirmation, nothing required; the row task shows the resolved brief |
| Ads | activate on the batch date without you; judged on their own first seven days; keep the lowest cost per lead from our leads among those Meta scaled (at least 150 riyals and 2,000 impressions in their own seven days); keep both unless the leader has at least 5 leads and a 20 percent margin; cost per click when leads are too few to compare; pause the rest, publish five new; nothing deleted; fatigue on the only scaled ad means keep and flag |
...
- **Suggest the three projects.** From units available, days since last featured, and last month's cost per qualified lead. The screen pre-fills; you confirm or swap one. Most months the choice is a glance.
- **Same three projects next month by default.** Rollover is the common case. The reminder says "keep these three?" with a yes.
- **Every row passes through you twice.** You have chosen not to cap approvals; counted as three per row that is nine slots a day, about six approval actions (36 row approvals and 120 paid approvals over 26 days), and it fits. It is still the one dependency the month cannot route around: a day you are away is a day nothing advances. A second approver for writing reviews, leaving you finals only, is the option if that ever matters. A capacity setting, not a role.
...
| Who schedules an approved post | nobody owns it | nobody; the month creates the releases at confirmation and the sweep publishes them at the batch slot, a task only when a person is needed |
...
| Your approvals | 228 a month across 6 surfaces | 18 row reviews and 18 row approvals for organic, plus the writing review and final approval of each paid creative, on one surface |
| Rail items | 19 | 6 |
| Settings screens in normal use | 16 | 1 |
| Definitions of "my work" | 4 | 1 |
| Exceptions | found by browsing | one list, one decision each |
| Reporting | 6 pages, paid numbers hand-keyed | the month page, real Meta spend and our WhatsApp leads |
...
│  │ آخر ظهور: أغسطس│ │ جديد          │ │ ت.ع.م. ٦٤ ر.س │                     │
│  └──────────────┘ └──────────────┘ └──────────────┘                     │
│                                                                          │
│  ماذا سيُنتَج                                                            │
...
│   الاعتماد  ٨٫٨ فتحة يوميًا (نحو ٦ اعتمادات) · بلا حد                     │
│                                                                          │
...
Each cell, each project column and the month itself take a note here, written before «اعتماد الشهر» (section 3.7); the ✎ marks show where. Before confirmation the page is a plan. After the 1st it is the same page with live numbers, past rows marked published, and the exceptions list doing the work. There is no other screen a normal month requires.
...
| Material rule: readiness refused at the designer's submit; files resolved at publish from the two slots by destination, caption from the approved writing, both checked against the approval hash; file ids and caption recorded on the publication after posting | missing; one check at submit, one at publish, one write-back |
...
| Weekly ranking on our leads, per ad over its own first seven days, keep both inside the guards, cost-per-click fallback, fatigue keep-and-flag | missing; the ranking exists but reads Meta's leads and a fixed window |
| The row task: one card to write and one to design, three records, one submit, writing review and final approval per row, send-back per post, order set by the writer and editable at approval, first-read post published last | missing |
...
| Sold-out replacement flow | missing; one exception with a project picker; the replacement takes over the remaining rows and slates |
| Carry-over of unfinished work into next month's first batch, counted against its capacity | missing; one rule in the month compiler |
| **Refresh decision applied automatically on the batch date; inside the guards both ads are kept, nothing escalated** | missing; one setting plus one rule |
...
| **The exceptions list** | missing; one query over the exception sources of section 3.3: incomplete row, rejected three times, ad failed, publish failed, account not connected, platform rejected, sold-out project, capacity breach |
