...
| B4 | `mos_campaign_plan_commit` | Commit four plans in one transaction (**D8**). Create `mos_content_rows` and their members. Write `account_id` (**S3**). Pin to the new versions (**A11**). Keep the advisory lock, the snapshot-hash check and the WS409 conflict contract exactly as they are — **never** SQLSTATE 40001/40P01. **✅ DONE 2026-09-15** — `supabase/migrations/2026-09-15_22_b4_month_commit_rows_and_pairs.sql`, applied to production. The unique key and the `ON CONFLICT` target moved together; `mos_campaign_plan_commit_month` commits the four plans in one transaction; `month_confirm`'s `rows_not_materialised` warning is gone because the state it described is no longer reachable. Proved on prod in a rolled-back transaction (16 rows / 48 members / 96 publications 48 feed + 48 story / 48 pairs / 80 row reservations / 3 slots on one day), with the baseline re-measured untouched afterwards. One thing found in passing: A3's `num_nonnulls(...) = 1` would have aborted every PAID plan, because 75 of each plan's 100 reservations belong to later-round creative slots that have no content shell until the sweep makes one — relaxed to `<= 1`, with the loud refusal moved into the RPC where it can name the key. | l | **high** |
