...
**Blocked tests — require a paired disposable WhatsApp number (human must scan QR / enter pairing code; cannot be done autonomously):** inbound/outbound text; image/video/audio/PDF send+receive; captions + quoted replies; ack lifecycle SERVER→DEVICE→READ on real messages; history backfill depth; duplicate/out-of-order events under real traffic; reconnect after network loss; two paired numbers simultaneously; connected-but-silent (zombie) detection window. **The POC instance is left running for exactly this:** dashboard at `https://waha-poc-wassell.fly.dev/dashboard` (user `wassell`, password in `.poc-secrets.local`), session `poc1` sitting at SCAN_QR_CODE. Scan with a **disposable** number and the message-level suite can run. Teardown when done: `fly apps destroy waha-poc-wassell waha-poc-catcher`.
...
2. **Webhook handler rework** — event names (`message`, `message.any`, `message.ack`, `session.status`), envelope shape, HMAC-SHA512 verification, dedupe on event id (duplicates are normal). DLQ + never-downgrade-ack logic carries over.
...
- **Functionally:** everything Wassell uses is covered except scheduled sends (must be built) and the Wassenger chat-status sugar (becomes CRM-owned, arguably cleaner). The POC on pinned `gows-2026.7.1` passed all 13 runnable infra tests, including retry-no-loss, restart persistence, and multi-session.
