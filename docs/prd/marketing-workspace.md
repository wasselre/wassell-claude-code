...
- **Meta is the source of truth for ADS (the execution's Ads tab).** After the
  ...
  **Sync from Meta** button on the execution header, `manage_paid_ads`) pulls the
  ...
  Meta/Instagram execution the Ads tab therefore shows **only the real synced ads**
  (`ExecutionDetailPage` `visibleAds` = rows with a `platform_ad_id`); the buyer's
