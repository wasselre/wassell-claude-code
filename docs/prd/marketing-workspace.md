...
- **Captions and ad copy are authored on the placement, split by purpose.** The
  content tab shows caption/ad-copy editors driven by the item's `purpose`, not a
  fixed set of boxes: an **organic** item shows one caption editor per organic
  platform it targets (`mos_content.organic_platforms`, picked inline), each
  bound to that platform's publication row — so the caption a writer types is the
  one that publishes; a **paid** item shows an ad-copy card per campaign execution
  (primary text / headline / description / CTA / destination URL) written onto the
  ad's `creative`; a **both** item shows both, clearly labelled. Shared hashtags
  live once on the content and are appended to every organic caption at publish.
