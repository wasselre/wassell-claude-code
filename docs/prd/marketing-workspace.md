...
- **«افتح المشروع» — open a record's project in Our Projects (2026-08-14).**
  ...
  (`/model/our_projects/:ourId`, the shared `ProjectDetailPage`), not the
  projects list. Marketing stores the `all_projects` master id, so the button
