...
- **The content table answers "what do I do on this row?" without opening it
  ...
  - **Phase derivation (`src/pages/Marketing/lib/stagePhase.ts`)** walks the
    ...
    *design* and every later step inherits that (so «مراجعة الكاتب» after
    ...
    `materials`, publish → `placements`; `ContentDetailPage` reads `?tab=` as
    its initial tab.
...
| `src/pages/Marketing/components/creative/` | Post Creative Director UI (2026-09-02): `CreativeTab` (flow + 4 s job polling + versioned edits + Apply/Revert/Regenerate), `TargetsPicker`, `ConceptCards`, `BaseCreativeEditor`, `SlideNavigator`, `DerivativesPanel` (full VisualAdaptation per target), `ReferencesPanel`, `AssetsPanel` (FilePickerModal replace with rights filters), `AiRecommendationsPanel`, `PalettePanel`, `WarningsPanel`, `VersionsBar`, `HandoffView` (copy + print), `labels.ts`, `__fixtures__/package.json` |
