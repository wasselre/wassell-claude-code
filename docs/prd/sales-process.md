...
**Status:** Live
**Last updated:** 2026-06-17 (**Next-Action on the client is now DB-trigger-maintained, not workflow-written.** The earlier note that the next-action trio "exists as fields but isn't denormalized / keeping them populated by the workflows is a follow-up" is corrected: `clients.next_followup_id` / `next_action_type` / `next_action_due_at` are recomputed by a SECURITY DEFINER Postgres trigger pair from the client's earliest open follow-up (mirroring the all_projects rollup pattern), read-only, self-healing — NOT by workflow `update_record` actions. Migration: `supabase/migrations/2026-06-17_client_next_action.sql`. See [clients.md](clients.md).)
