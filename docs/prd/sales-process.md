...
Below the activities, an expanded stage also shows a **"Stage automations" (أتمتة المرحلة)** block: workflows tagged to the stage (`metadata.sales_stage`) that aren't tied to a specific follow-up activity — e.g. the time-based **Auto-close appointment as No-Show after 24h** (`on_due`) under موعد زيارة. Each row shows the workflow name, a trigger badge (e.g. "Automatic · when due"), an Inactive/Advanced pill where relevant, and the same Open-in-Builder / Runs links. Without this block a stage-level automation was invisible in the Studio (it only incremented the stage card's linked count, never rendering).
