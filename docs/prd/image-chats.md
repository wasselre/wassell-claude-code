...
**Status:** ⛔ ARCHIVED (2026-07-22) — removed from the UI in the dormant-module cleanup; sessions, media assets, and worker preserved
**Last updated:** 2026-07-22 (**Module archived — dormant-module cleanup (commit `203f410`).** The Image Studio was removed from the UI: `image_chats` and `image_presets` (plus the Designs-suite siblings) were added to `ARCHIVED_MODULE_MODELS` in `src/lib/featureFlags.ts` — sidebar entries hidden, deep links land on the "section archived" notice (`src/components/RetiredAssistantNotice.tsx`). Deleted code: `src/pages/ImageChats/**`, `src/lib/imageChat/**`, `src/lib/assets/**`, and the `api/image-chat/*` endpoints (`send`, `generate`, `promote-asset`). **KEPT:** `api/_lib/imageGen.ts` (shared fal.ai adapter — still used by `api/icons/generate` + `api/marketing/generate`), the Fly worker's image pipeline (`worker/src/runImageJob.ts` + `worker/src/imageGen.ts` — the `generation_jobs` queue simply stays empty), and the live clean-text + video-convert worker jobs for Listing Messages. **Non-destructive to data:** the `image_chats` / `image_presets` / `prompt_snippets` models and records, the `media_assets` table, the `generation_jobs` queue, promoted `files` rows, and all `marketing-assets` bytes remain in Supabase. **Restore path:** `git revert 203f410` + remove the model names from `ARCHIVED_MODULE_MODELS`. Everything below documents the feature as it existed when archived.) | Previously 2026-06-18 (composer prompt draft auto-saved per session to localStorage + restored on mount, so a refresh / app update no longer loses what you were typing; the "your work in this session" generation strip moved from above the composer to a collapsible bar pinned at the TOP, below the header — header shows count + a live "N running" indicator, collapse state persisted; composer prompt-length guard: 4,000-char cap enforced client-side BEFORE the optimistic textarea reset — over-long prompts now toast + keep the text instead of clearing it, plus a live character counter; v3 — rebuilt as a Creative **Workspace** (Higgsfield-style), not a chat: Generations + a first-class media-asset library; v3.2 — full-screen viewer redesigned: covers the app sidebar, LEFT = collapsible Branding/Design prompt sections, RIGHT = actions; the Media Library opens the same viewer)
...
- **"Your work in this session" bar** — a **collapsible** strip of generation thumbnails pinned at the **top** (below the header), NOT above the composer. Its header shows the generation count plus a live "N running" spinner while jobs are in flight (visible even when collapsed); click the header to expand/collapse (preference persisted in `localStorage`, per-user-global); click a thumbnail to load that generation into the canvas.
...
- **Concurrency safety:** image turns append to a SHARED `messages` array, and rapid sends + the worker's fill-in are concurrent writers. Every write uses optimistic concurrency (`record_save` `p_expected_version` + retry on the WS409 version-mismatch; the `records_bump_version` trigger increments `version` on each UPDATE), so the loser of a race re-applies onto the latest array instead of clobbering. That makes ANY worker count safe — the deck worker app runs **5 machines** and image jobs ride on them. See CLAUDE.md "Generation jobs pipeline" hard rules.
...
- The right-pane composer is **bottom-pinned** and persistently visible. Its controls:
  ...
  2. **Textarea** + prompt-library button (📚). The draft is **auto-saved per session to `localStorage`** (`wassell_image_chat_draft_<sessionId>`) on every keystroke and restored on mount — a refresh or an app update (full SPA reload) no longer loses what you were typing; the draft is cleared once the prompt is successfully sent. Prompts are capped at **4,000 characters** (mirrors the server guard). As the user nears the cap a live `current / 4,000` counter appears (red once over); Send disables and a Send attempt is **blocked with a non-destructive toast that preserves the typed text** — the textarea is never auto-cleared on a too-long prompt.
...
> **Archive note (2026-07-22, commit `203f410`):** every `src/pages/ImageChats/**`, `src/lib/imageChat/**`, `src/lib/assets/**`, and `api/image-chat/*` row below was **DELETED** (restore via `git revert`) — as were the v3 files listed above (`StudioWorkspace.tsx`, `SelectedAssetPanel.tsx`, `AssetLightbox.tsx`, `AssetActions.tsx`, `MediaLibraryModal.tsx`, `lib/generations.ts`, `api/image-chat/generate.ts`). The `api/_lib/imageGen.ts`, `worker/**`, migration, and seed rows still exist.
...
| `src/pages/ImageChats/components/Composer.tsx` | Bottom composer. `atCapacity` gates only Send (never the textarea); consumes a one-shot reference/variation `seed`. Per-session prompt draft persisted to `localStorage` (`sessionId` prop) + restored on mount, cleared on send. Enforces the 4,000-char prompt cap before the optimistic reset (toast + keep text), with a live counter. |
