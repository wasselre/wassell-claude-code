...
- **Home Screen install (iOS)** — the app is a installable web app:
  ...
  - **Push notifications (added 2026-07-29)** — alerts arrive on the phone even when the app is closed:
    ...
    - Three events are pushed:
      ...
      - **New WhatsApp message (added 2026-08-18)** — «💬 رسالة واتساب جديدة», sent to the **assigned salesperson** the moment a customer's inbound message lands. Body is the client's name plus a preview (media messages show «صورة» / «رسالة صوتية» / … instead); tapping it opens that conversation at `/model/chats/:conversationId`.
    ...
    - **Assigned-salesperson resolution** (each rung falling through to the next; nothing resolved ⇒ **no push**, never an error): the conversation's mirrored `client_owner` → the linked client's own `client_owner` → a phone match against Clients (only when the conversation record does not exist yet, because the webhook writes the message before it upserts the conversation) → the chat's own `owner` assignee. Most inbound traffic is on chats with no linked client at all, and those stay silent on purpose.
    - **Bursts collapse.** One `tag` per conversation means the phone replaces the previous alert for that chat instead of stacking; a `dedupe_key` bucketed to 5 minutes of the message's own timestamp means five messages in a row create ONE queue row. Reactions, system events, outbound echoes, and anything older than 15 minutes (a history import) never enqueue at all.
    - **Admins can switch each built-in off** at `/profile → الإشعارات` (workspace-wide, admin-only — `push_builtin_settings`), so a hand-built Workflow notification rule can replace a built-in instead of competing with it. The WhatsApp alert additionally steps aside when the same message is about to fire a **customer reply** for the same rep and client, so one event never buzzes twice.
...
- **The WhatsApp-inbound alert stays silent for a conversation with no linked client** (roughly 40% of inbound traffic as of 2026-08-18, and a further slice where the linked client has no assigned owner). That is deliberate — guessing a recipient would be worse — but it means "I never got notified" can be a *linking* problem rather than a push problem. Check `records.data.client_link` / `client_owner` on the conversation first.
