...
- **Home Screen install (iOS)** — the app is a installable web app:
  ...
  - **Push notifications (added 2026-07-29)** — alerts arrive on the phone even when the app is closed:
    ...
    - Two events are pushed, matching the in-app watcher exactly: a **hot lead** (a first booking call for a brand-new client) and a **customer reply**. Escalated leads, unassigned tasks, and repeat booking calls for the same client deliberately do not fire.
    - Tapping a notification focuses the open app and routes to the follow-up rather than reloading it.
    - Server-side by necessity: the `records_enqueue_push` trigger writes to `push_outbox` and the Fly worker delivers. Postgres cannot call out directly — neither `pg_net` nor `pg_cron` is installed.
...
| `supabase/migrations/2026-07-29_web_push.sql` | `push_subscriptions`, `push_outbox`, the enqueue trigger, worker RPCs |
