...
| `src/pages/Dashboard/components/widgets/{Stat,BarChart,PieChart,LineChart,Table,Funnel,Leaderboard,Gauge,Progress,Pivot,Heatmap}Widget.tsx` | Presentational renderers (one per family) |
...
- **Map** is the one widget type not built — it needs Saudi region geo-data (district/city → coordinates or GeoJSON), a product decision still open. A KPI-target card was folded into Stat + comparison; per-target On-Track/Off-Track lives in the future `KPIDefinition` layer, not a widget.
