...
- **Sales Operating System fields (added 2026-06-17).** The client record is the spine of the guided sales lifecycle. Beyond the existing stage/status fields, it now carries:
  ...
  - **`next_followup_id`** (lookup → follow-ups), **`next_action_type`** (call / whatsapp / appointment / offer / financing / ownership), **`next_action_due_at`** — the denormalized "what's next" for this client.
  - **`last_activity_at`** — last touch timestamp.
  - **`lost_reason`** + **`lost_at`** — why and when a client became Lost (the same lost-reason option set the follow-up uses).
  - **`lifecycle_health`** (on_track / overdue / no_next_action / closed) — a health flag for dashboards.

  **These are maintained by the Sales Lifecycle workflows, not edited by hand.** A rep records an outcome in the Follow-up Workspace, and the bound workflow advances `client_stage` / `client_status` and creates the next task. The Sales Queue and Sales Manager compute the live "needs a next action" picture from the open follow-up set. See [sales-process.md](sales-process.md) and [followups-workspace.md](followups-workspace.md).
- **Extended stage/status taxonomy (added 2026-06-17).** `client_stage` gained two **side-exit** stages — **خاسر (Lost)** and **مغلق ناجح (Closed Won)** — joining the existing lifecycle stages. `client_status` gained ten new Arabic values used along the lifecycle: رقم خاطئ (Invalid Number), مكرر (Duplicate), تم رفض العرض (Offer Rejected), يحتاج معلومات تمويل (Needs Financing Info), نقاش عائلي (Family Discussion), بانتظار القرار (Waiting Decision), تم إرسال واتساب (WhatsApp Sent), إعادة تواصل لاحقًا (Recontact Later), بارد (Cold), بانتظار دفعة الحجز (Waiting Reservation Payment). Stored values equal the Arabic labels (canonical). A client in any terminal/side-exit stage (Unqualified / Lost / Closed Won) is excluded from the "no next action" health metric.
