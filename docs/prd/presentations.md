...
**Last updated:** 2026-04-27 (Phase 3.4.2 — Template Assistant gained a `propose_template_patch` tool. The assistant can now FILL template fields directly: basics, slug, description, icon, tools, input_schema, steps (kind + prompt + tool subset), and output_structure. Each tool call streams as a `proposal` SSE event and renders inline in the chat as an Apply / Discard card; clicking Apply merges the patch into the local draft via `applyAssistantPatch` (which generates ids on new steps/inputs/dropdown options and constrains step.tools to the template's enabled tools). Saving still requires the user to click Save. The system prompt was also updated to brief the model on every recent platform change (brands first-class, output_structure split, Lucide icons, format-on-template). Phase 3.4.1 — card icons switched from typographic Unicode glyphs (♪ ◆ ✦ etc) to Lucide line icons in the slide's accent color (copper on white/cream cards, gold on brown/dark cards); platform-tile logos for Snapchat / TikTok / Instagram / LinkedIn keep their official multi-color brand marks. Slide format / dimensions (`16:9 (10" × 5.625")`) moved out of the brand into the template's `output_structure` — format is per-deck, not cross-template visual identity. Phase 3.4 — split slide structure out of the brand into a new free-form `output_structure` text field on the template. Brand now carries only cross-template visual identity (palette, typography, RTL/digit/punctuation rules, banned vocabulary); `output_structure` carries per-deck slide layout (count, sequence, footer rules per slide, slide-specific required phrases). The cloud worker injects both blocks into every step's user message — brand spec first (`## Brand spec`), output structure second (`## Output structure`). Editor adds an "Output structure" textarea section between Brand and Steps. Phase 3.3 — brands promoted to first-class entities (`presentation_brands` table + `brand_id` FK on templates). Editing a brand updates every template that references it; queue path resolves `brand_id` and freezes the brand body into `template_snapshot.brand` so in-flight jobs are immune. Phase 3.1 — per-step Retry button on the detail page; store action `retryPresentationStep` resets the target step + everything downstream to `pending` and re-queues the job; cloud worker's resume path skips already-`completed` steps. Mid-step progress hooked into the agent loop's `onStep`. Step prompts support `{{input_name}}` interpolation. Verified: re-running just step 2 of a prior run took 28.9 s vs 50.7 s for the full pipeline.)
...
| `src/pages/Presentations/TemplateEditorPage.tsx` | **Phase 2 + 3.3 + 3.4 + 3.4.2** — `/presentations/templates/:templateId`. Sections: Basics, Record binding, Inputs, Tools, Brand (selector — see `BrandSelector.tsx`), **Output structure** (free-form markdown textarea — count, format, sequence, footer rules per slide, slide-specific required phrases; injected into every step's prompt), Steps. Read-only banner for daemon-synced templates. Save commits to store + Supabase. **Phase 3.4.2** adds `applyAssistantPatch` — the AI assistant's `propose_template_patch` proposals merge into the draft via this function, which generates ids on new steps / inputs / dropdown options and constrains step.tools to the template's enabled tools. |
| `src/pages/Presentations/components/template/TemplateAssistantPanel.tsx` | **Phase 2 + 3.4.2** — Right-side drawer with streaming chat. Renders both prose (with code-fence detection) and Apply / Discard cards for each `proposal` SSE event. Each card shows the patch summary, a chip list of which fields would change, and Apply / Discard buttons. Once acted on, the card flips to a status chip ("Applied" / "Discarded"). |
| `src/lib/templateAssistant/client.ts` | **Phase 2 + 3.4.2** — SSE client for `/api/template-assistant`. Now parses three event types: `text` (token deltas), `proposal` (a `TemplateProposal { id, summary, changes }`), and `done`/`error`. `buildTemplateContext` projects the template draft into the wire shape sent on every turn. |
| `api/template-assistant.ts` | **Phase 2 + 3.4 + 3.4.2** — Vercel edge function that streams Anthropic's `messages.stream` API. Carries a static cached system prompt (architecture, the 5 tools, brand vs output_structure split, recent platform updates) plus a per-turn template-context system block. **Phase 3.4.2** wires a custom `propose_template_patch` tool with full input schema for every patchable field; the handler runs a manual agent loop (cap 4 rounds), emits a `proposal` SSE event per tool call, and feeds a synthetic `tool_result` back so the model can keep talking after a patch. |
