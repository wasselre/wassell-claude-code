...
**Status:** Closed loop with per-step retry. User-authored templates run end-to-end; failed or stale steps can be re-run individually without redoing earlier work.
**Last updated:** 2026-04-26 (Phase 3.1 — per-step Retry button on the detail page (visible on completed/failed/skipped steps when the parent job is terminal); store action `retryPresentationStep` resets the target step + everything downstream to `pending` and re-queues the job, while the cloud worker's resume path detects already-`completed` steps in the seeded `step_outputs` and skips them. Mid-step progress hooked into the agent loop's `onStep` so the detail page sees "Step 2/2: build → drive_upload (iter 1)"-style messages live. Step prompts support `{{input_name}}` interpolation for any of the template's user inputs. Verified: re-running just step 2 of a prior run took 28.9 s vs 50.7 s for the full pipeline.)
...
| `src/pages/Presentations/TemplateEditorPage.tsx` | **Phase 2** — `/presentations/templates/:templateId`. Five sections: Basics (slug, labels, description, icon), Record binding, Inputs, Tools, Steps. Read-only banner for daemon-synced templates. Save commits to store + Supabase. |
...
| `src/types/index.ts` | `PresentationInput`, `PresentationTemplate` (now with `tools` / `steps` / `is_user_authored`), `PresentationToolName`, `PresentationStepKind`, `PresentationStep`, `PresentationJob`, `DaemonStatus`, job status / error enums |
| `src/stores/appStore.ts` | State + actions — Phase 2 adds `createPresentationTemplate`, `updatePresentationTemplate`, `deletePresentationTemplate`, `duplicatePresentationTemplate` |
...
| `supabase/schema.sql` | Tables `presentation_templates` (extended with `tools` / `steps` / `is_user_authored` / `created_by`), `presentation_jobs`, `daemon_status` + idempotency unique index + `claim_next_presentation_job` RPC + idempotent `ALTER TABLE ADD COLUMN IF NOT EXISTS` for existing prod DBs |
...
| `cloud-worker/src/runner.ts` | **Phase 3** — when `template.steps` is non-empty, walks each step in order, persists per-step state to `step_outputs`, passes earlier-step output as context to later steps, scrapes `drive_url` from any `drive_upload` tool calls. Falls back to the Phase 1B hardcoded smoke prompt only when steps are empty (legacy daemon-synced templates). **Phase 3.1** — seeds `stepOutputs` from the job row's existing `step_outputs` so re-queued jobs skip steps already marked `completed`. Step prompts support `{{input_name}}` interpolation; the agent loop's `onStep` hook is wired to write per-iteration progress messages mid-step. |
