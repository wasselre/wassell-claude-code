...
3. **Provider routing differs between local and production.** In production `qwenRoutingEnabled()` is false for this endpoint's environment (no Qwen log lines; every live post came back `generated_by: anthropic:claude-opus-4-7`), so prod runs on Anthropic — verified working end-to-end on 2026-07-27. Locally the reverse held: the dev `ANTHROPIC_API_KEY` is out of credits, so the local integration tests ran on Qwen with `TEXT_LLM_PROVIDER=qwen`. Both paths are therefore proven, but note that a prompt tuned against one provider is not automatically validated on the other. Setting `TEXT_LLM_PROVIDER=qwen` in Vercel would move production onto the cheaper model.
