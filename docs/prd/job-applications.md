...
- **Cost calculator** (added 2026-09-08, same day): the offer card projects what the candidate would cost us. Inputs: base salary, rep commission % (a share of the COMPANY's commission), sales per month, average sale price (default 1,250,000 SAR), company commission % (default 2.5 %). Live output: company commission per sale, rep commission per sale, rep commission per month, **monthly cost = salary + commission**, annual cost (× 12). The scenario inputs persist with the offer so the projection is reproducible. Internal only, never printed on the letter. Inputs accept Arabic-Indic digits, `%`, and thousands separators; an invalid number turns the field red and blocks save/PDF.
- **Offer letter PDF** (added 2026-09-08): «ملف العرض PDF» builds a branded A4 letter client-side (`src/lib/careers/offerPdf.ts`, same rasterize recipe as the units PDFs) and downloads it: Wassel header, greeting by name + phone, position, base salary, commission wording («X٪ من عمولة الشركة عن كل عملية بيع»), work location (Riyadh, on-site, 6 days), optional **التفاصيل والمكافآت** block (toggle «تضمين التفاصيل والمكافآت في الملف», default on when details exist), closing and two signature lines. If the card has unsaved edits, generating the PDF saves them first. The projection numbers are never included.
...
- **Recruitment-experience link** (added 2026-09-17): the drawer's «إرسال رابط التجربة عبر واتساب» mints a per-candidate `invite_token`, **auto-sends the personal `/careers/experience/<token>` link over WhatsApp**, and sets `offer_sent`. The candidate confirms their name/phone, goes through the guided experience, and picks interested/declined at the end — which writes `offer_accepted` / `offer_rejected` + `experience_decline_reason` back to this row automatically. The drawer shows the link, confirm state, and their decision. See [recruitment-experience.md](recruitment-experience.md). Endpoints: `api/careers/experience.ts` (public, token-authed) + `api/careers/send-experience.ts` (admin-gated); migration `supabase/migrations/2026-09-17_job_applications_experience_link.sql`.
...
5b. **Offer (internal):** after the interview → set `إعداد العرض` → fill salary / commission % / sales-per-month (and adjust the price / company-commission assumptions if needed) → read the projected monthly + annual cost → write details/bonuses → «حفظ العرض» → optionally «ملف العرض PDF» (tick/untick including the details) → send the letter to the candidate outside the app (WhatsApp/call) → set `تم إرسال العرض` (send time stamped) → on the candidate's answer set `قبِل العرض` (then `تم التوظيف`) or `رفض العرض`.
...
- **Admin writes (internal):** `job_applications.status`, `offer_salary`, `offer_commission`, `offer_details`, `offer_sent_at`, `offer_sales_per_month`, `offer_avg_sale_price`, `offer_company_commission_pct`, `review_notes` via the admin's JWT under the existing unrestricted admin UPDATE policy (no RLS change was needed for the offer stage).
...
| `src/lib/careers/offerPdf.ts` | Candidate-facing branded A4 offer letter (client-side jsPDF + html2canvas) |
...
| `supabase/migrations/2026-09-08_02_job_applications_offer_calc.sql` | Adds the cost-projection inputs `offer_sales_per_month` / `offer_avg_sale_price` / `offer_company_commission_pct` |
