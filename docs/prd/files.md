...
- **Bulk select (Google Drive style).** Each tile shows a circular checkbox in the corner — hover-revealed when nothing is selected, always visible once selection is active. Click the checkbox to toggle without opening the file. A rubber-band marquee is available by mouse-dragging from any empty area of the grid; Shift adds to the existing selection, Ctrl/Cmd toggles symmetric-difference. Plain-clicking a tile when something else is selected replaces the selection with just that one tile. Keyboard: `Esc` clears, `Ctrl/Cmd+A` selects every visible tile. The selection survives marquee-drag releases but is cleared when the user navigates between folders or tabs. A sticky bulk-action bar appears at the top showing `N selected` and `Download` / `Move` / `Delete` buttons; each button is auto-disabled when the user has no permission for any item in the selection (delete + move both gate on per-item ownership). Folder bulk-move is intentionally not exposed yet — the bar's Move button only addresses the file rows in the selection.
...
| `src/pages/Files/components/BulkActionBar.tsx` | Sticky toolbar shown when ≥1 item is selected. Download / Move / Delete / Clear. |
