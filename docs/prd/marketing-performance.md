...
- **Placement** *(replaced 2026-09-17)*: `mos_perf_place_open_task` is now a thin alias for `mos_task_dispatch` — see the dispatcher note in the header. `mos_role_load` intake and `mos_role_sla` no longer decide assignment or deadlines; `mos_step_rules` does.
- **SLA = ONE working day, Friday-skipped (2026-08-30).** Since the team has no fixed shift, a short wall-clock SLA (4h) unfairly flagged tasks late overnight. Now **every task gets one full working day and no more**: `due_at = mos_perf_due_after(opened_at, LEAST(COALESCE(sla,24),24))`, where `mos_perf_due_after` adds elapsed hours but **Friday (the weekly off day) contributes zero** — so a Thursday-afternoon task is due Saturday, never mid-Friday, and a task never falls due on the off day. Approved leave still pauses on top. All `mos_role_sla` rows are set to 24 and the placement caps at 24, so the SLA grid reads the truth. `production_days_per_week=6` (Friday off) already matches. Existing open tasks had their `due_at` recomputed under the rule.
- **Orphan guard (2026-08-30):** the late sweep no longer flags OPEN workflow tasks whose `mos_content` subject was deleted — an orphan can never be worked or closed, so flagging it late forever was a bug. The existing orphan tasks were skipped and their observe-mode discipline records cleared (this was the entire «قرارات معلّقة» pile for ريان: 9 orphan tasks on deleted content).
