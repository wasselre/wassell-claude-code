...
- **Writes:** `records.data` for the follow-up (the outcome + completion fields + snapshots) via `saveRecord`; `records.data` for the **client** when the rep saves inline preference edits (the five preference fields); a new appointment record when "Book Appointment" is used; a new WhatsApp conversation + first message when the rep sends from the in-app composer; `linked_followup_id` back onto the attached phone_call record. All client stage/status moves and next-task creation are written by the **workflow engine**, not this page. The objective text shown in the mission is read (not written) from the merged sales-process config — manager edits to it are written from the Sales Manager page, not here.
...
| File | What it does |
|---|---|
| `src/pages/Followups/FollowUpWorkspacePage.tsx` | The Workspace page — layout, draft state, Complete & Save, bidirectional evidence stamp, the appointment modal, the WhatsApp composer + client-preview modals, the merged-config (overrides) wiring, the Advanced-Fields link |
