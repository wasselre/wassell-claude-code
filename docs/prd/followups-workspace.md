...
- **Evidence linking (bidirectional).** For call outcomes the `EvidencePicker` suggests the client's recent **outbound** phone calls (matched by `client_link` or by canonicalized phone) and lets the rep attach one as the call that completed this follow-up (`completed_by_call_id`). On Complete & Save, if a call was attached, the Workspace also stamps `linked_followup_id` back onto that phone call record — so the link is visible from both sides. (See [calling.md](calling.md).) Attaching evidence is encouraged via a warning but not hard-required.
...
| `src/pages/Followups/components/EvidencePicker.tsx` | Suggests recent outbound calls + attaches one as `completed_by_call_id` |
