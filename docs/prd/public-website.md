...
- `search_public_projects(p_lang, p_filters jsonb)` → `{ projects[], total, facets{districts,bedrooms,bathrooms,amenities,public_total}, geo }`. Filters are AND-ed: `districts[]` (project `location.district`), `bedrooms[]` + `bedrooms_min`, `bathrooms[]` + `bathrooms_min`, `amenities[]` (project must have ALL — `preferred_amenities ?& …`), `budget_min/max`, `geo_items[]`. Unit-level filters keep a project only if ≥1 **non-sold** unit matches; `best_unit` = available first (then under construction, reserved), cheapest, with a plan. Facet counts are over the whole public set so chips never vanish once picked. Geo items are compiled with the CRM's own `wassell_compile_geo_items` under a throwaway client id and matched against `project_points` (include-union minus exclude, direction via `wassell_geo_dir_match`, district-id fallback), then the temp rows are deleted — same posture as the already-anon `wassell_preview_geo_items`. `geo.needs_review > 0` means a rule was ignored (the page shows a notice). Measured 0.5–0.9 s per call as anon (units are materialized once per call and joined on the `(model_id, data->>'project_id')` expression index).
