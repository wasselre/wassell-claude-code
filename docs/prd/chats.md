...
**Status:** Live
**Last updated:** 2026-05-07 (**Realtime + RLS hardening, deployed 2026-05-07.** `chat_messages`'s RLS posture tightened in Phase B-followup: SELECT for `authenticated`, no INSERT/UPDATE/DELETE for users — every write goes through `service_role` from `/api/webhook/haberchat.ts`. The pre-existing `Authenticated full access` USING(true) policy is gone. The webhook handler continues to use the service-role Supabase client (`api/_lib/supabaseServer.ts`) so it bypasses RLS as before. **The `records` table is now on the `supabase_realtime` publication** (Phase C.1, with `REPLICA IDENTITY DEFAULT` — PK only) — the previous "Records table not on Realtime" limitation is closed for the chats use case: when the webhook handler bumps a chats parent record (`last_message_at` / `last_message_preview`), the `RealtimeOrchestrator`'s `mergeRecord` handler refetches the row by id and applies it to every open browser. Earlier `subscribeToAllChats` continues to drive `chat_messages` updates; the parent-record bump is now handled by the orchestrator's records channel, not the chat-specific subscription. Replica identity for `chat_messages` is `FULL` (Phase A.5) so UPDATE/DELETE payloads carry the whole row.)
**Related PRDs:** [navigation-layout.md](navigation-layout.md), [data-storage.md](data-storage.md), [record-management.md](record-management.md), [access-control.md](access-control.md)
