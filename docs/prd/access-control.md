...
3. **Create a user:** `/settings/users` → "+ New User" → fill name + email → pick profile (defaults to first; empty option removed) → check any roles and fill their field values inline → save.
...
| `src/components/layout/Header.tsx` | User-switcher (no null escape hatch) |
...
- **No real auth** — identity is simulated via the header user-switcher. No passwords, no sessions.
- **No row-level permissions** — can't say "user X can only see records they own".
