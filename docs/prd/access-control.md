...
- **Route guards:** `<RequireAdmin>` wraps admin-only routes (Builder, Workflows, Dashboards, Translations, Profiles, Roles, Users settings) — it also enforces TOTP MFA (aal2). `<RequirePageAccess pageId>` wraps the three `/sales/*` routes — same redirect-home-with-toast posture as RequireAdmin but WITHOUT the MFA step (these are operational surfaces meant to be grantable to non-admin sales staff who may legitimately be at aal1). Both are UX gates; server-side RLS remains the trust boundary.
...
| `src/hooks/usePermission.ts` | `usePermission`, `useModelPermissions`, `useIsAdmin`, `useCanAccessPage` |
| `src/lib/permissions.ts` | Permission evaluation + `isAdmin` + `canAccessPage` |
