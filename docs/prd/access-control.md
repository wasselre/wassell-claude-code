...
| `src/lib/moduleDependencies.ts` | `resolveModelDataDependencies(rootModelId, models)` — pure transitive closure of the modules a model displays data from (walks `lookup_model_id` across the schema graph; mirror/section_mirror covered via their sibling lookup). Drives the editor's auto-reference grants; NOT an enforcement path |
