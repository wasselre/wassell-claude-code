...
| `src/lib/permissions.ts` | Permission evaluation + `isAdmin` + `canAccessPage` + `canViewWorkflows`; all functions take an optional trailing `previewProfileId` honored only for users with `can_preview_profiles`; exports `resolveEffectiveProfile` |
