...
**Last updated:** 2026-06-25 (**Lookup-first matching with dual-read (Saudi geography layer):** the engine now resolves the requested district to the authoritative `districts` record (id + real centroid) via a new `resolveRequestedDistrict` helper, and a project whose `district_lookup` equals the resolved id is an EXACT relational match. The old text fuzzy-match on `preferred_neighborhoods` + per-project averaged centroids is RETAINED as the dual-read fallback for projects not yet backfilled. New geo helpers in `src/lib/locationUtils.ts`: `pointInPolygon`, `pointInDistrict`, `distanceBetweenCentroids`, `haversineKm`, `normalizeArabicDistrictName`, `normalizeEnglishDistrictName`. See [record-management.md](record-management.md) and [data-storage.md](data-storage.md).) | 2026-06-21
