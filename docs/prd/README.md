...
| 30 | [project-matching-assistant.md](project-matching-assistant.md) | Project Matching Assistant (Phase 1) — live-call sales co-pilot. Customer requirements → ranked best-fit projects + ready-to-say pitch. Deterministic two-tier search (our_projects first, all_projects fallback flagged for verification) + weighted TEXT scoring in code, AI narrates only over verified facts. Custom split-pane chat (`matching_chats`). No geo yet (Phase 2). Key files: `api/match.ts`, `api/_lib/matchAgent.ts`, `src/pages/Matching/**`, `src/lib/matching/**`, `supabase/migrations/2026-06-18_matching_chats_model.sql` |
