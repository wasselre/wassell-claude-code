...
export default async function handler(req: Request): Promise<Response> {
  ...
  return withAuth(req, async () => {
    ...
    if (llmRoutingEnabled()) {
      try {
        const out = await llmText({
          track: { area: 'internal', callSite: 'api/project-ai' },
        ...
        logLlmFallback('/api/project-ai', err);
      }
    }
    ...
    const client = trackedAnthropic(new Anthropic({ apiKey }), { area: 'internal', callSite: 'api/project-ai', isFallback: llmRoutingEnabled(), fallbackFrom: llmRoutingEnabled() ? 'deepseek' : null });
    try {
