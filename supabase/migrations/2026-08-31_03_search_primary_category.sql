-- business_files_search learns the new required PRIMARY "Document Type"
-- (files.primary_category): as a FILTER and as a RETURNED field on every row.
--
-- Deliberately NOT added as a FACET count. The five metadata-axis facets were
-- removed on 2026-08-23 because each one re-scans the RLS-gated `base` CTE and
-- together they tipped the unfiltered all-files query over statement_timeout.
-- primary_category would be one more such pass; the filter-bar dropdown for it
-- is served from the vocabulary (file_vocabularies) instead, which needs no
-- scan. Adding the facet is the same single-pass rewrite the other five wait on.
--
-- Full re-emit of the live function (2026-08-25_01) with ONLY the three
-- primary_category additions marked below — kept self-contained so a fresh DB /
-- CI builds the same behavior.
...
DECLARE
  ...
  -- ADDED: primary "Document Type" filter (single-select axis; multiple values = OR).
  ...
  WITH base AS (
    SELECT fi.id, fi.title, fi.document_type, fi.kind, fi.origin, fi.status,
     ...
     WHERE fi.file_class = v_class
       ...
       -- ADDED: primary "Document Type" filter.
  ...
  SELECT jsonb_build_object(
    ...
    'rows', coalesce((
      SELECT jsonb_agg(jsonb_build_object(
               ...
               -- ADDED: the required primary "Document Type" on every row.
    ...
    'facets', jsonb_build_object(
      ...
      -- usage_rights / production_state / subject / primary_category) are
      -- intentionally NOT computed here — each is a re-scan of the RLS-gated
      -- `base` CTE, and the batch of them tipped the unfiltered query over the
      -- statement timeout (live incident 2026-08-23). Filtering on them works
      -- (WHERE clauses above + URL params + the vocab-driven dropdowns); the
      -- facet COUNTS wait on the single-pass rewrite (the B2 optimization).
