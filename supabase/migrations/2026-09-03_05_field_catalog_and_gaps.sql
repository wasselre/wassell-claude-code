-- ============================================================================
-- Phase 1 · 04 · Source-field catalog, authoritative mappings, schema-gap events
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  END LOOP;
END $$;
-- The reconciled-status view is owned by the definer and inherits the base-table
-- RLS (security_invoker default = false → but base tables are admin-gated; grant
-- SELECT only, admins are the effective readers via the underlying policies).
