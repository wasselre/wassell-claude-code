-- File Metadata Intelligence — Phase B: extend business_files_search with the
-- new axes (asset_nature / acquisition_source / usage_rights / production_state)
-- + multi-value subject, as filters AND facets, and return them on each row.
--
-- Reproduces the live function verbatim (fetched via pg_get_functiondef) with
-- ONLY the metadata additions; the filter/sort/paging/health logic is unchanged.
-- No AI here — ai_description is returned but never written by search.
...
BEGIN
  ...
  WITH base AS (
    SELECT fi.id, fi.title, fi.document_type, fi.kind, fi.origin, fi.status,
           ...
           fi.ai_description, fi.width_px, fi.height_px, fi.duration_seconds, fi.page_count
