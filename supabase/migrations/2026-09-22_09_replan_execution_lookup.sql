-- ============================================================================
-- Plan-driven task assignment — 9: a re-plan finds the campaign's EXISTING
-- execution (2026-09-22).
--
-- Found by the rolled-back dry-run of the September re-plan: both
-- mos_plan_resolve_carry and mos_campaign_plan_commit looked a campaign's
-- execution up by "platform + EMPTY label". The month's live Meta executions
-- are labelled («WSL · 2026-09 — مدفوع — …», stamped by the Meta push), so
-- the lookup found nothing: the carry matched no started paid creative (their
-- old bookings were counted as occupied capacity on top of the new ones and
-- the guard refused), and the commit would have INSERTED a second, unlinked
-- execution per paid campaign with new cycles and slots under it.
--
-- One shared lookup now: an explicit execution_id in the payload; else the
-- campaign's live execution on that platform with an empty label; else, when
-- the campaign has EXACTLY ONE live execution on that platform, that one.
-- Ambiguity (several labelled executions) still yields NULL and the commit
-- still creates a fresh execution — the pre-existing behaviour.
--
-- Both functions re-emitted from their LIVE definitions with only this change.
-- ============================================================================

BEGIN;

CREATE OR REPLACE FUNCTION public.mos_campaign_execution_for(p_campaign_id uuid, p_platform text)
  ...
  SELECT COALESCE(
    (SELECT x.id FROM public.mos_campaign_executions x
      WHERE p_campaign_id IS NOT NULL AND x.campaign_id = p_campaign_id
        AND x.platform = p_platform AND COALESCE(x.label, '') = '' AND x.archived_at IS NULL
      ORDER BY x.created_at LIMIT 1),
    (SELECT min(x.id::text)::uuid FROM public.mos_campaign_executions x
      WHERE p_campaign_id IS NOT NULL AND x.campaign_id = p_campaign_id
        AND x.platform = p_platform AND x.archived_at IS NULL
     HAVING count(*) = 1));
$$;
  ...
  WITH execs AS (
    SELECT e ->> 'key' AS ekey,
           COALESCE(NULLIF(e ->> 'execution_id', '')::uuid,
                    public.mos_campaign_execution_for(p_campaign_id, e ->> 'platform')) AS eid
...
BEGIN
  ...
  FOR e IN SELECT * FROM jsonb_array_elements(COALESCE(p_materialise -> 'executions', '[]'::jsonb)) LOOP
    ...
    IF v_id IS NULL AND v_campaign IS NOT NULL THEN
      v_id := public.mos_campaign_execution_for(v_campaign, e ->> 'platform');
    END IF;
