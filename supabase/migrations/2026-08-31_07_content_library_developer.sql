-- Competitor Content Library — surface the developer link.
-- Every developer competitor is already linked to a record in our `developers`
-- model (mkt_organizations.developer_record_id). Return it per row so each
-- content item can open its developer's CRM record, like Files links to a project.
-- NULL for marketer-published posts (their developer comes via the project — step 2).
-- Additive; carries the media fields from _04 forward.
