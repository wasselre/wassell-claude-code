-- ============================================================================
-- Plan-driven task assignment — 4/6: RE-PLAN = retire, carry forward, guard once.
-- 2026-09-22. Plan v9 §6 + 07-decisions.md D3 ("remove whatever we had before
-- and re-plan; delete it and change the dates"), D10, D11, D16.
--
-- ONE phased transaction per commit:
--   0  lock, hash gate, resolve the campaign
--   1  retire every replaced plan's unconsumed bookings; CARRY the live
--      bookings the new plan re-books (rows by row_key, creatives by
--      content_key within the campaign, virtual creatives by cycle): started
--      work keeps its dates, unstarted work takes the new dates (D16)
--   2  ONE capacity guard over the union of proposed load MINUS carried
--      entries, over every cell the plan touches INCLUDING carried bookings'
--      destinations (existing from the ledger alone; same-day iff row_key)
--   3  materialise + insert only the non-carried bookings
-- `mos_campaign_plan_commit_month` runs phases 1–2 for the whole month before
-- any per-plan insert (batch mode suppresses the per-plan retire+guard).
-- No 40001/40P01; conflicts are WS409 with the cells named (D11).
-- ============================================================================

BEGIN;

-- ── carry-forward resolution: which payload entries map to a LIVE booking ───
CREATE OR REPLACE FUNCTION public.mos_plan_resolve_carry(p_campaign_id uuid, p_plan_id uuid,
                                                         p_reservations jsonb, p_materialise jsonb)
 RETURNS TABLE(idx integer, item_key text, row_key text, step_key text, subject_key text,
               reservation_id uuid, res_status text, res_plan_id uuid, res_task_id uuid)
 LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  ...
  SELECT r.idx, r.item_key, r.row_key, r.step_key, r.subject_key, res.id, res.status, res.plan_id, res.consumed_task_id
    ...
    JOIN public.mos_task_reservations res
      ON res.step_key = r.step_key AND res.status IN ('bound','consumed')
...
-- ── retirement of replaced plans (never the carried rows, never started work) ─
CREATE OR REPLACE FUNCTION public.mos_retire_superseded(p_campaign_id uuid, p_keep_plan_id uuid, p_by_plan uuid,
                                                        p_carry_ids uuid[] DEFAULT '{}')
 RETURNS jsonb LANGUAGE plpgsql SECURITY DEFINER SET search_path TO 'public' AS $$
DECLARE v_repl uuid[]; v_retired int := 0; v_closed int := 0; v_plans int := 0; v_carry uuid[] := COALESCE(p_carry_ids, '{}');
BEGIN
  ...
  -- 1a unconsumed bookings → history
  ...
  RETURN jsonb_build_object('retired', v_retired, 'tasks_closed', v_closed, 'plans', v_plans);
END $$;

-- ── ONE capacity guard ──────────────────────────────────────────────────────
-- proposed = the payload entries (same-day iff row_key; spread as the ledger
-- spreads); extra cells = destinations of carried bookings (they contribute 0
-- proposed — they are already in `existing` at their new dates through the
-- ledger — but their cells are checked against the slots and against leave).
CREATE OR REPLACE FUNCTION public.mos_capacity_check(p_reservations jsonb, p_extra_cells jsonb DEFAULT '[]'::jsonb)
...
/** Destination cells of the carried BOUND entries of a payload (the bound rows adopt the new dates). */
CREATE OR REPLACE FUNCTION public.mos_plan_carried_cells(p_reservations jsonb, p_carried_idx int[])
 RETURNS jsonb LANGUAGE sql STABLE SECURITY DEFINER SET search_path TO 'public' AS $$
  SELECT COALESCE(jsonb_agg(jsonb_build_object('user_id', x.r ->> 'assignee_user_id', 'bucket', COALESCE(x.r ->> 'bucket', 'post'), 'day', s.day)), '[]'::jsonb)
    FROM (SELECT r, (ord - 1)::int AS idx FROM jsonb_array_elements(COALESCE(p_reservations, '[]'::jsonb)) WITH ORDINALITY t(r, ord)) x
    CROSS JOIN LATERAL public.mos_spread_effort_mode(GREATEST((x.r ->> 'planned_start')::date, public.mos_perf_today()),
                                                    COALESCE((x.r ->> 'weight')::numeric, 1),
                                                    NULLIF(x.r ->> 'row_key', '') IS NOT NULL) s
   WHERE x.idx = ANY (p_carried_idx) AND NULLIF(x.r ->> 'assignee_user_id', '') IS NOT NULL;
$$;

/** The payload minus the carried entries (what Phase 2 counts as proposed load). */
CREATE OR REPLACE FUNCTION public.mos_plan_proposed_minus(p_reservations jsonb, p_carried_idx int[])
 RETURNS jsonb LANGUAGE sql IMMUTABLE AS $$
  SELECT COALESCE(jsonb_agg(r ORDER BY ord), '[]'::jsonb)
    FROM jsonb_array_elements(COALESCE(p_reservations, '[]'::jsonb)) WITH ORDINALITY t(r, ord)
   WHERE NOT ((ord - 1)::int = ANY (COALESCE(p_carried_idx, '{}')));
$$;

-- ── the per-plan commit ─────────────────────────────────────────────────────
DROP FUNCTION IF EXISTS public.mos_campaign_plan_commit(uuid, jsonb, text, jsonb, uuid);
CREATE OR REPLACE FUNCTION public.mos_campaign_plan_commit(p_plan_id uuid, p_reservations jsonb, p_expected_hash text,
                                                           p_materialise jsonb, p_actor uuid,
...
BEGIN
  ...
  IF NOT p_batch_mode THEN
    ...
    -- Phase 2: ONE guard — proposed minus carried, over every touched cell
    v_cells := public.mos_capacity_check(
                 public.mos_plan_proposed_minus(p_reservations, v_carry_idx),
                 public.mos_plan_carried_cells(p_reservations,
                   (SELECT COALESCE(array_agg(x.idx), '{}') FROM public.mos_plan_resolve_carry(v_campaign, p_plan_id, p_reservations, p_materialise) x
                     WHERE x.res_status = 'bound')));
...
-- ── the month: phases 1–2 over the WHOLE batch, then per-plan inserts ───────
...
DECLARE
  ...
  v_carry_idx int[]; v_carry_ids uuid[]; v_bound_idx int[];
  v_union  jsonb := '[]'::jsonb;
  v_extra  jsonb := '[]'::jsonb;
  v_cells  jsonb;
  v_retire jsonb := '[]'::jsonb;
BEGIN
  ...
  FOR e IN SELECT * FROM jsonb_array_elements(COALESCE(p_plans, '[]'::jsonb)) LOOP
    ...
    SELECT COALESCE(array_agg(x.idx), '{}'), COALESCE(array_agg(x.reservation_id), '{}'),
           COALESCE(array_agg(x.idx) FILTER (WHERE x.res_status = 'bound'), '{}')
      INTO v_carry_idx, v_carry_ids, v_bound_idx
    ...
    v_union := v_union || public.mos_plan_proposed_minus(COALESCE(e -> 'reservations', '[]'::jsonb), v_carry_idx);
    v_extra := v_extra || public.mos_plan_carried_cells(COALESCE(e -> 'reservations', '[]'::jsonb), v_bound_idx);
  END LOOP;

  -- Phase 2: ONE guard
  v_cells := public.mos_capacity_check(v_union, v_extra);
  IF jsonb_array_length(v_cells) > 0 THEN
    RAISE EXCEPTION 'capacity_conflict'
