...
-- Guarded so the CI ephemeral DB (which has no frozen `districts` table — the
-- bootstrap fixture only stubs the tables the translation migrations touch)
-- skips this prod data-fix instead of erroring on a missing relation.
DO $$
BEGIN
  IF to_regclass('public.districts') IS NULL THEN
    RAISE NOTICE 'districts table absent (CI) — skipping UAE name_en backfill';
    RETURN;
  END IF;
