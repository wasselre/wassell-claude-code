-- ============================================================================
-- Phase 1 · 03 · Multi-artifact immutable raw capture
-- ----------------------------------------------------------------------------
...
-- the definer publisher/adapter RPCs (migration 08, owner-run). No UPDATE/DELETE
-- grants anywhere (also blocked by the append-only trigger).
DO $$
DECLARE t text;
BEGIN
  FOREACH t IN ARRAY ARRAY['raw_blobs','raw_snapshots','raw_snapshot_artifacts','page_capture_manifest'] LOOP
...
-- ---------------------------------------------------------------------------
-- Raw-artifact Storage: private bucket + a dedicated NON-BYPASS uploader role.
-- Enforceability note: service_role has BYPASSRLS, so an "insert-only" guarantee
-- must NOT depend on the worker using the service_role key. The worker uploads
-- with a scoped JWT whose role = market_raw_uploader (a NOLOGIN, NOBYPASSRLS role
-- assumable via `authenticator`), which can INSERT content-addressed objects but
-- has no UPDATE/DELETE policy. Immutable to all application/ingestion roles;
-- mutable only via postgres/service_role infra authority (no routine worker holds
-- those for market-raw). Gate-B must verify the current Storage version honours a
-- custom role JWT claim; fallback is an INSERT-only edge signer.
...
DO $$ BEGIN
  ...
  END IF;
END $$;
GRANT market_raw_uploader TO authenticator;

-- INSERT only, for the uploader role only (content-addressed key ⇒ upsert=false ⇒
-- an existing hash is a no-op, never an overwrite). NO update/delete policy exists.
DROP POLICY IF EXISTS market_raw_upload ON storage.objects;
CREATE POLICY market_raw_upload ON storage.objects FOR INSERT TO market_raw_uploader
  WITH CHECK ( bucket_id = 'market-raw' );

-- READ: admin only, via signed URLs from an admin RPC. No anon, no blanket authenticated.
